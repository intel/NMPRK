...
IpmiSession::sendRecv(timeval timeoutVal, bool bSend, 
					 bool bRecv, bool& timeout, 
					 const dcmi_req_t& req, dcmi_rsp_t& rsp, bool inSession)
{
    ByteVector serializedReq;
    ByteVector serializedRsp;
	memset(&rsp,0, sizeof(dcmi_rsp_t));
	if(bSend)
	{
...
void
IpmiSession::sendRecv(const dcmi_req_t& req, dcmi_rsp_t& rsp, bool inSession)
{
    ByteVector serializedReq;
                 ...
                 confidAlgo(confidAlgo), useKg(false)
{
	...
	session_send_sol = false;
    LOG_LODEBUG << "Creating RMCP+ IPMI Session [authAlgo: " << authAlgo
        << ", integAlgo: " << integAlgo << ", confidAlgo: " << confidAlgo << "]";
    *reinterpret_cast<unsigned int*>(sidm) = 0x85be2329;
}

void RmcppIpmiSession::getDcmiCapInfo(dcmi_rsp_t& rsp)
{
...
void
RmcppIpmiSession::getChannelAuthCap(dcmi_rsp_t& rsp)
{
    LOG_LODEBUG << "Get Channel Auth Cap (RMCP+)...";
...
RmcppIpmiSession::openSession()
{
    ...
    LOG_LODEBUG << "    Max Priv Level: 0x" << hex << (int)maxPrivLevel;

    LOG_LODEBUG << "    Sent Console ID:     " << sidm;
    LOG_LODEBUG << "    Returned Console ID: " << osRsp.sidm;
    ...
    LOG_LODEBUG << "    Management System Session ID: " << sidc;

    //Authentication Payload
    LOG_LODEBUG << "    Authentication: " << (ipmi_auth_algo)osRsp.authPayload.algorithm;
    //Integrity Payload
    LOG_LODEBUG << "    Integrity: " << (ipmi_integ_algo)osRsp.integPayload.algorithm;
    //Confidentiality Payload
    LOG_LODEBUG << "    Confidentiality: " << (ipmi_confid_algo)osRsp.confidPayload.algorithm;
}

void
RmcppIpmiSession::rakp12(ByteVector& randC, ByteVector& randM, Guid& guidC, byte_t& roleM, ByteVector& sik)
{
    LOG_LODEBUG << "  RAKP 1-2";
    ...
    LOG_LODEBUG << "    PWD: " << kUid;
    LOG_LODEBUG << "    Digest: " << digest;
    ...
    LOG_LODEBUG << "      Matches Mng Digest";
...
RmcppIpmiSession::rakp34(const ByteVector& randC, const ByteVector& randM, const Guid& guidC, byte_t roleM, const ByteVector& sik)
{
    ...
    LOG_LODEBUG << "    RAKP4: Managed ID OK";
    ...
    if (AUTH_ALGO_RAKP_NONE != authAlgo)
    {
        ...
        LOG_LODEBUG << "    RAKP4: " << digestSize << "-bytes digest OK";
    }
}
...
{	
    ...
    LOG_LODEBUG << "    Channel number: 0x" << hex << setw(2) << setfill('0') << (int)gcacRsp.channelNumber;
    LOG_LODEBUG << "    Authentication Type Support: 0x" << hex << setw(2) << setfill('0')
        << (int)gcacRsp.authTypeSupport.value;
    LOG_LODEBUG << "    Authentication status: 0x" << hex << setw(2) << setfill('0') << (int)gcacRsp.authStatus.value;
    LOG_LODEBUG << "    Extended Capabilieties: 0x" << hex << setw(2) << setfill('0') << (int)gcacRsp.extendedCaps.value;
    ...
    LOG_LODEBUG << "Session established";
...
void
RmcppIpmiSession::serializeRequest(ByteVector& out, const dcmi_req_t& req, bool inSession)
{
    // Header
...
void
RmcppIpmiSession::deserializeResponse(dcmi_rsp_t& rsp, const ByteVector& in)
{
    size_t pos = 0;
...
void
RmcppIpmiSession::serializeIntRequest(ByteVector& out, ipmi_payload payloadType, const dcmi_req_t& req)
{
...
void
RmcppIpmiSession::sendRecvInt(ipmi_payload reqPayloadType, const dcmi_req_t& req, 
...
RmcppIpmiSession::verifyRmcppIntegrity(const ByteVector& in)
{
   ...
   signedDataLength = (signedDataLength + 3) & 0xfc;
    ...
    if (INTEG_ALGO_MD5_128 == integAlgo)
    {
        ...
        LOG_LODEBUG << "MD5 sum correct on incoming message";
    }
    else
    {
        ...
        if (memcmp(digest.c_ptr(), in.c_ptr(4 + signedDataLength), HMAC_PROPERTIES[integAlgo].integSize) != 0)
		{
			...
			LOG_LODEBUG << "Hmac on incoming message correct";
		}
    }
}
